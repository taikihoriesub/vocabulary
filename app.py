...
def get_csv_files_from_repo(repo_url, repo_dir):
    ...
    return [f[:-4] for f in os.listdir(repo_dir) if f.endswith('.csv')]
...
def file_selection_session():
    ...
    if not st.session_state['data_loaded']:
        ...
        if st.button('Load File'):
            selected_file = selected_file_name + '.csv'
...
def setup_session():
    if st.session_state['data_loaded']:
        ...
        test_types = ['English to Japanese (Multiple Choice)', 'Japanese to English', 'English to Japanese (Descriptive)']
...
def question_session():
    if st.session_state['test_index'] < len(st.session_state['test_questions']):
        ...
        question = question_data['English'] if 'English to' in st.session_state['test_type'] else question_data['Japanese']
        correct_answer = question_data['Japanese'] if 'English to' in st.session_state['test_type'] else question_data['English']

        if 'Multiple Choice' in st.session_state['test_type']:
            if st.session_state['test_index'] not in st.session_state['user_choices']:
                other_choices = [row[st.session_state['test_type'].split(' to ')[1].split(' (')[0]] for row in st.session_state['test_questions'] if row != question_data]
