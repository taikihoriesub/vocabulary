...
def initialize_session_state():
    ...
    if 'learning_or_testing' not in st.session_state:
        st.session_state['learning_or_testing'] = None
...
# テストか学習を選ぶセッション
def learning_or_testing_selection_session():
    choice = st.radio("Choose an option", ["Learning", "Test"])
    if st.button('Confirm Choice'):
        st.session_state['learning_or_testing'] = choice
        if choice == "Test":
            st.session_state['test_state'] = 'file_selection'
        else:
            st.session_state['test_state'] = 'learning'

# 学習セッション
def learning_session():
    chapter = st.selectbox('Choose a Chapter to Learn', st.session_state['chapter_data']['Chapter'].unique())
    if st.button('Show Words'):
        filtered_data = st.session_state['chapter_data'][st.session_state['chapter_data']['Chapter'] == chapter]
        st.table(filtered_data[['English', 'Japanese']])
    if st.button('Done Learning'):
        st.session_state['test_state'] = 'learning_or_testing'
...
def main():
    ...
    if st.session_state['test_state'] == 'learning_or_testing':
        learning_or_testing_selection_session()

    elif st.session_state['test_state'] == 'learning':
        learning_session()

    elif st.session_state['test_state'] == 'file_selection':
