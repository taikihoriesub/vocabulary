...
def setup_session():
    if st.session_state['data_loaded'] and st.session_state['chapter_data'] is not None:
        ...
        if chapter_list.size > 0:
            # last_chapterが存在するか確認し、存在しなければchapter_listの最初の要素を使用
            default_chapter = st.session_state.get('last_chapter')
        ...
        num_words = st.number_input('Number of words to test', min_
